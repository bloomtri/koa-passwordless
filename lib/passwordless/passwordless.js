...
Passwordless.prototype.requestToken = function(getUserID, options) {
  ...
  return async function(ctx, next) {
    ...
    if(typeof user === 'string' && user.length === 0) {
      ...
      ctx.throw(401,'Provide a valid User');
    } else if(!deliveryMethod || !user) {
      // return sendError(400);
      ctx.throw(400,'No user field');
    }
    ...
    getUserID(user, delivery, function(uidError, uid) {
      if(uidError) {
        ...
        self._tokenStore.storeOrUpdate(token, uid.toString(), ttl, origin, async function(storeError) {
          if(storeError) {
            await next(new Error('Error on the storage layer: ' + storeError));
          } else {
            deliveryMethod.sendToken(token, uid, user, async function(deliveryError) {
              if(deliveryError) {
                await next(new Error('Error on the deliveryMethod delivery layer: ' + deliveryError));
              } else {
                if(!req.passwordless) {
                  req.passwordless = {};
                }
