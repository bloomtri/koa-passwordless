    ...
    if(token && uid) {
      self._tokenStore.authenticate(token, uid.toString(), function(error, valid, referrer) {
        if(valid) {
          ...
          if (error === 'expired') {// date expired error case
            req[self._userProperty] = uid;
            req.session.passwordless = req[self._userProperty];
            self._tokenStore.invalidateUser(uid, function (err) {
              if (err) {
 ...
 */
Passwordless.prototype.requestToken = function(getUserID, options, customCtxObject) {
  var self = this;
  options = options || {};
  return async function(ctx, next) {
    ...
    if(!req.fields && !options.allowGet) {
      throw new Error('req.fields does not exist: did you require middleware to accept POST data (such as body-parser) before calling acceptToken?')
    ...
    if(req.fields && req.method === "POST") {
      user = req.fields[userField];
      delivery = req.fields[deliveryField];
      if(originField) {
        origin = req.fields[originField];
      }
    ...
    if(typeof user === 'string' && user.length === 0) {
      ...
      throw new Error(401,'Provide a valid User');
    } else if(!deliveryMethod || !user) {
      // return sendError(400);
      throw new Error(400,'No user field');
    }

    await getUserID(user, delivery, async function(uidError, uid) {
      if(uidError) {
         throw new Error('Error on the user verification layer: ' + uidError);
      } else if(uid) {
        var token;
        try {
          if(deliveryMethod.options.numberToken && deliveryMethod.options.numberToken.max) {
           ...
           throw new Error('Error while generating a token: ' + err);
        }
        var ttl = deliveryMethod.options.ttl || 60 * 60 * 1000;
        await self._tokenStore.storeOrUpdate(token, uid.toString(), ttl, origin, async function(storeError) {
          if(storeError) {
             throw new Error('Error on the storage layer: ' + storeError);
          } else {
            await deliveryMethod.sendToken(token, uid, user, async function(deliveryError) {
              if(deliveryError) {
